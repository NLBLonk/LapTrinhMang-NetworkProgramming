    ...
    public class AsyncSocketTCPClient
    {
        ...
        public async Task ConnectToServer()
        {
            ...
            try
            {
                ...
                _ = ReadDataAsync(mClient);
            }
            catch (SocketException sex)
            {
                Console.WriteLine($"Socket Error: {sex.Message}");
                throw new Exception($"Cannot connect to server: {sex.Message}");
            }
            catch (Exception exp)
            {
                Console.WriteLine($"Connection Error: {exp.Message}");
                throw new Exception($"Connection failed: {exp.Message}");
            }
        }
        ...
        private async Task ReadDataAsync(TcpClient mClient)
        {
            ...
            catch (IOException ioex)
            {
                ...
                throw new Exception("Connection lost with server.");
            }
            catch (Exception excp)
            {
                Console.WriteLine($"Read Data Error: {excp.Message}");
                throw new Exception($"Error reading data: {excp.Message}");
            }
        }
